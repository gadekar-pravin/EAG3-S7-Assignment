...
Vendored from llm_gatewayV7/client.py so the agent repo has no filesystem
dependency on the gateway source tree. The gateway runs as a separate service;
this module talks to it over HTTP.
"""
...
class LLM:
    ...
    def stream(
        ...
        *,
        ...
        with httpx.stream("POST", url, json=body, timeout=self.timeout) as r:
            r.raise_for_status()
    ...
    def embed(
        self, text: str, task_type: str = "retrieval_document", provider: str | None = None
    ) -> dict:
